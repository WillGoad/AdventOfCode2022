...
def flatAppend(array, arrayToAppend):
    newArray = array
    for i in range(len(arrayToAppend)):
        newArray.append(arrayToAppend[i])
    return newArray
...
def craneInstruction(array, size, start, end):
    ...
    newArray = array
    newArray[end] = flatAppend(newArray[end],newArray[start][-1*size:])
    newArray[start] = newArray[start][:len(newArray[start])-size]
    return newArray
